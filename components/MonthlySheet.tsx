...
export const MonthlySheet: React.FC<Props> = ({ userId, employees, sites, setEmployees }) => {
    ...
    // Funzione helper per salvare in cloud non bloccante
    const syncToCloud = (monthKey: string, data: MonthlyData) => {
        console.log('[SYNC] Saving monthly data to Firestore:', { userId, monthKey, overridesCount: Object.keys(data.overrides || {}).length });
        setSaveStatus('saving');
        saveMonthlyData(userId, monthKey, data)
            .then(() => {
                console.log('[SYNC] Monthly data saved OK');
                setSaveStatus('saved');
            })
            .catch(err => {
                console.error('[SYNC] ERROR saving monthlyData:', err);
                setSaveStatus('saved');
            });
    };

    const syncRecurringToCloud = (jobs: Record<string, ExtraJob[]>) => {
        setSaveStatus('saving');
        saveRecurringJobs(userId, jobs)
            .then(() => setSaveStatus('saved'))
            .catch(err => {
                console.error("Errore salvataggio recurringJobs:", err);
                setSaveStatus('saved');
            });
    };
    ...
    const handleDownloadExcel = () => {
        ...
        // Add BOM to Blob for UTF-8 compatibility in Excel
        const blob = new Blob(['\ufeff', csvContent], { type: 'text/csv;charset=utf-8;' });
        const url = URL.createObjectURL(blob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `Presenze_${format(currentDate, 'MMMM_yyyy', { locale: it })}.csv`;
        link.click();
    ...
    const handleDownloadPDF = () => {
        ...
        allTds.forEach(td => {
            ...
            // Replace the entire td content with a split layout: dividing line between work hours and permit
        ...
        autoTable(doc, {
            html: clone,
            theme: 'grid',
            startY: 20, // Leave space for the text header
            styles: {
                fontSize: 7,
    ...
    const COL_W_RATE = "w-[90px] min-w-[90px]";
    const COL_W_TARGET = "w-[100px] min-w-[100px]";
                            ...
                            <tr className="text-white">
                                ...
                                <th className={`${TH_BASE} ${COL_W_TARGET} bg-cyan-600 p-0`}>
                                    <div className="flex flex-col items-center justify-center leading-none py-1">
                                        <span className="text-white text-[10px]">NETTO</span>
                                        <span className="text-white text-[10px] mt-1 pt-0.5 border-t border-cyan-500/50 w-[80%] text-center">LORDO</span>
                                    </div>
                                </th>
